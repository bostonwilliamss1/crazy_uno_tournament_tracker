...
function PreviousTours() {
    ...
    const [tournamentsInformation, setTournamentsInformation] = useState<Tournament[]>();

    useEffect(() => {
        axios.get('http://localhost:5001/api/tournaments-titles')
            .then((response) => {
    ...
    useEffect(() => {
        axios.get('http://localhost:5001/api/tournaments')
